...
app.use(morgan("dev")); // أو "combined" للتفاصيل الكاملة
...
app.post('/compress', upload.array('images'), async (req, res) => {
    ...
    const crop = req.body.crop === '1';
    const keepTransparency = req.body.keep_transparency === '1';

    const convertTo = req.body.convert_to ? req.body.convert_to.split(',') : []; 
    ...
    for (const file of req.files) {
        ...
        try {
            ...
            // خلفية إذا لم نحافظ على الشفافية
            if (!keepTransparency) {
            ...
            if (convertTo.length > 0) {
                for (let format of convertTo) {
                    ...
                    if (fmt === 'jpeg' || fmt === 'jpg') {
                        await image.jpeg({ quality: qualityJPEG }).toFile(outPath);
                    } else if (fmt === 'png') {
                        await image.png({ quality: qualityPNG, compressionLevel: 9 }).toFile(outPath);
                    } else if (fmt === 'webp') {
                        await image.webp({ quality: qualityWebP }).toFile(outPath);
                    } else if (fmt === 'avif') {
                        await image.avif({ quality: qualityAVIF }).toFile(outPath);
                    }
                ...
                // نفس الصيغة الأصلية فقط
                if (ext === '.png') {
                    await image.png({ quality: qualityPNG }).toFile(outputPath);
                } else {
                    await image.jpeg({ quality: qualityJPEG }).toFile(outputPath);
                }
